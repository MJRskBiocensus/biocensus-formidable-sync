...
require_once __DIR__ . '/class.pack.database.build.iterator.php';
/**
...
class DUP_PRO_Database
{
    ...
    public function __construct(AbstractPackage $package)
    {
        ...
        $this->DBMode            = DUP_PRO_DB::getBuildMode();
     ...
     * @return string
     */
    public function getTempSafeFilePath()
    {
        return SnapIO::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . "/{$this->File}");
    }
    ...
    private function runMysqlDump($exePath): bool
    {
        ...
        if (file_put_contents($this->dbStorePathPublic, $sql_header, FILE_APPEND) === false) {
            DUP_PRO_Log::error("file_put_content failed", "file_put_content failed while writing to {$this->dbStorePathPublic}");
            return false;
        }
    ...
    {
        DUP_PRO_Log::trace('WRITING CREATES/INSERTS VIA STREAM');
    ...
    private function openSqlFile()
    {
        ...
        $handle = @fopen($path, 'a');
        if (! $handle) {
            DUP_PRO_Log::error('Cannot open SQL file', $path);
        }
        return $handle;
    }
    ...
    private function processLine(
        ...
        if (fwrite($fileHandle, $line) === false) {
            DUP_PRO_Log::error('fwrite failed', $this->dbStorePathPublic);
            $hadWriteError = true;
        }
    }
    ...
    private function getMysqlDumpCmd(
        ...
        $cmd .= ' --allow-keywords';
        $cmd .= ' --net_buffer_length=' . SnapUtil::getIntBetween($global->package_mysqldump_qrylimit, Constants::MYSQL_DUMP_CHUNK_SIZE_MIN_LIMIT, Constants::MYSQL_DUMP_CHUNK_SIZE_MAX_LIMIT);
        $cmd .= ' --no-tablespaces';
     ...
     * @return string[]
     */
    private function getFilteredTables(bool $getExcludedTables = false)
    {
        global $wpdb;
        $result = [];
        // ALL TABLES
     ...
     * @return void
     */
    public function startTableIteratorCallback(DUP_PRO_DB_Build_Iterator $iterator): void
    {
     ...
     * @return void
     */
    public function endTableIteratorCallback(DUP_PRO_DB_Build_Iterator $iterator): void
    {
            ...
            do {
                ...
                if (($lastSelectNumRows = SnapDB::numRows($result)) > 0) {
                    while (($row = SnapDB::fetchAssoc($result))) {
                        ...
                        $line             .= '(' . implode(',', array_map(['DUP_PRO_DB', 'escValueToQueryString'], $row)) . ')';
     ...
     * @return void
     */
    private function setError($message, $fix, $quickFix = false): void
    {
        DUP_PRO_Log::trace($message);
        $this->Package->build_progress->failed = true;
        DUP_PRO_Log::trace('Database: buildInChunks Failed');
        $this->Package->update();
        DUP_PRO_Log::error("**RECOMMENDATION:  $fix.", $message);
     ...
     * @return string[]
     */
    private static function getTablesFilteredFromValidation()
    {
    ...
    private function doFiltering(): void
    {
        ...
        $tables          = $this->getFilteredTables();
        $tablesToProcess = array_map(['DUP_PRO_DB', 'updateCaseSensitivePrefix'], $tables);
    ...
    private function writeInsertChunk(): void
    {
        ...
        $traceLogEnabled = DUP_PRO_Log::isTraceEnabled();
    ...
    private function doFinish(): void
    {
        ...
        DUP_PRO_Log::info("SQL FILE SIZE: " . SnapString::byteSize($sql_file_size));
